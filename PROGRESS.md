...
- Learned the basic **client-server model**, where a client sends a request and a server listens for, processes, and responds to that request.
- Understood the practical flow of a web request:
...
- Understood that a browser and `curl` can act as clients, while a web server such as Django or Python's built-in HTTP server acts as the server.

#### IP Addresses and Network Interfaces

- Learned the purpose of **IP addresses** in identifying network endpoints.
- Learned the difference between IPv4 and IPv6 addresses.
- Understood the **loopback interface**:
  - IPv4: `127.0.0.1`
  - IPv6: `::1`
- Learned that `10.x.x.x` addresses belong to the private IPv4 address space.
- Learned CIDR notation using `/24`, which corresponds to `255.255.255.0`.
- Learned basic Linux interface states such as `UP`, `LOWER_UP`, `DOWN`, and `NO-CARRIER`.
- Used `ip addr` to inspect the available network interfaces and their addresses.

#### Ports, Sockets, TCP and UDP

- Learned that a **port** identifies a service endpoint on a host.
...
- Learned the basic difference between **TCP** and **UDP**:
  ...
  - UDP is connectionless and provides lower-overhead datagram delivery without TCP's reliability guarantees.
- Learned that the Python HTTP server and Django development server use TCP for HTTP connections.
- Used `ss` to inspect listening TCP and UDP sockets.
- Learned how `ss -ltnp` can show which process owns a listening TCP socket.
- Understood the practical relationship:
...
- Learned that `ping` uses **ICMP**, not HTTP, TCP, or UDP.
- Used:
...
- Learned the meaning of `icmp_seq`, `ttl`, round-trip time, and packet loss.
- Observed successful IPv6 connectivity to Google with an average RTT of approximately `55.7 ms`.
- Understood that successful DNS resolution and network reachability are separate concepts.
...
- Learned:
  ...
  - DNS responses contain a **TTL** used for caching.
  - DNS TTL is different from the IP packet TTL seen in `ping`.
  - `NOERROR` indicates that the DNS query was successfully answered.
- Learned the difference between `getent` (OS name-service resolution) and `dig` (direct DNS query inspection).

#### HTTP Request and Response

- Started a Python HTTP server using:
...
- Learned that an HTTP response consists of:
...
- Used `curl -I` to inspect response headers without downloading the normal response body.
- Observed headers such as `Content-Type`, `Content-Length`, `Server`, and `Date`.
- Learned that binding the server to `0.0.0.0` makes it listen on all IPv4 interfaces, while `127.0.0.1` restricts it to the local machine.
- Accidentally started the server from the home directory and observed the directory listing, which demonstrated the importance of choosing the correct serving directory and interface.

#### Django and HTTP in Practice

- Ran the Django development server using:

```bash
python manage.py runserver 3000
```

- Tested the Django feedback endpoint using:

```bash
curl -i http://127.0.0.1:3000/feedback/
```

- Observed a real HTTP response from Django including status, headers, cookies, and generated HTML.
- Connected the previous Django concepts with the network layer:
...
- Observed the Django-generated CSRF token directly in the HTTP response, connecting the earlier CSRF lesson with an actual request/response.

#### CSRF Inspection in the HTTP Response

- Inspected the actual HTML and HTTP headers returned by Django for the feedback form.
- Observed the hidden CSRF field generated by `{% csrf_token %}`:

```html
<input type="hidden" name="csrfmiddlewaretoken" value="...">
```

- Observed that Django also returned a `Set-Cookie` header containing the `csrftoken` cookie.
- Learned that the token visible in the form is a **masked representation** and is not simply the same string as the underlying CSRF secret stored in the cookie.
- Understood the validation flow:

```text
Browser GET /feedback/
        ↓
Django sets CSRF cookie
        ↓
Template renders hidden CSRF token
        ↓
Browser submits POST + CSRF token + cookie
        ↓
...
- Learned that missing or invalid CSRF tokens cause Django to reject the unsafe request, normally with HTTP `403 Forbidden`.
- Understood that for HTTPS requests Django can also use `Origin` / `Referer` checks as an additional CSRF defense.
- Observed `SameSite=Lax` on the CSRF cookie in the HTTP response.
- Observed the default CSRF cookie lifetime in the response (`Max-Age=31449600`, approximately one year).
- Learned that the CSRF secret is normally kept in the cookie rather than regenerated for every GET request, and Django rotates the CSRF secret during login.
- Understood why the browser's **Same-Origin Policy** matters: an attacker site can try to cause a cross-site request, but normally cannot read the protected response or obtain the target site's CSRF token.
- Tested the reasoning with the Postman scenario: Postman can submit a valid CSRF-protected request only when it has obtained and supplied the required valid cookie/token pair.
- Clarified that **CSRF protection is not authentication**. A valid CSRF token does not identify the user or replace login/session credentials.
- Understood that CSRF is designed to protect against forged browser requests that take advantage of automatically included authentication state, while attacks such as XSS or stolen credentials/tokens are different security problems.

#### Linux DNS Resolution and systemd-resolved
...
- Learned that DNS configuration can exist at both global and connection-specific levels.
- Observed that the active USB/tethering connection supplied its own DNS server.
- Understood why a globally configured DNS server does not necessarily mean every NetworkManager connection will automatically ignore its own DNS configuration.
- Used `tcpdump` to inspect DNS-related traffic and distinguish local resolver/stub traffic from upstream DNS traffic.

#### DNS Leak Investigation and DNS-over-TLS

- Investigated whether DNS queries were being sent to the ISP's DNS resolver.
- An extended DNS leak test showed an address associated with **Equinix**, rather than the ISP.
- Learned that seeing Equinix can be consistent with Quad9 infrastructure and does not by itself indicate an ISP DNS leak.
- Verified the active Quad9 protocol using:
...
- Learned that `dot` indicates **DNS-over-TLS (DoT)**.
- Confirmed that the tested DNS query path was using encrypted DNS-over-TLS through Quad9.

### Tasks / Activities

- Started and tested Python's built-in HTTP server.
- Used `curl` to send HTTP requests and inspect responses.
- Ran the Django development server and inspected its actual HTTP response.
- Inspected Linux network interfaces with `ip addr`.
- Tested network connectivity using `ping`.
- Inspected TCP and UDP sockets using `ss`.
- Identified listening processes using `ss -ltnp`.
- Investigated hostname resolution using `getent` and `dig`.
- Inspected DNS configuration using `resolvectl status`.
- Captured DNS-related traffic using `tcpdump`.
- Inspected the raw Django HTML and HTTP headers to understand how CSRF protection appears on the wire.
- Compared the CSRF cookie with the masked hidden form token and traced the request-validation flow.
- Reasoned through cross-site request forgery scenarios and the role of the browser Same-Origin Policy.
- Investigated the DNS leak status of the current system configuration.
- Verified Quad9 DNS-over-TLS using the Quad9 protocol test.

### Challenges

- Understanding the difference between an IP address, port, socket, and process.
- Understanding why a global DNS configuration can differ from the DNS server supplied by an active network connection.
- Understanding the difference between DNS TTL and IP packet TTL.
- Understanding the difference between local DNS stub traffic and upstream DNS traffic.
- Troubleshooting DNS behavior after changes to `/etc/resolv.conf` and the resolver configuration.
- Determining whether an Equinix address shown by a DNS leak test represented an ISP leak or Quad9 infrastructure.
- Understanding why the CSRF token in the HTML and the CSRF cookie are related but not identical values.
- Understanding how a cross-site attacker can potentially send a request but normally cannot read the protected response/token because of browser same-origin restrictions.

### Key Takeaways

- A client communicates with a server using a network endpoint identified by an IP address and port.
- HTTP normally runs over TCP, while `ping` uses ICMP.
- DNS is a separate service used to resolve domain names before many network connections are made.
- `ip`, `ss`, `ping`, `dig`, `getent`, `resolvectl`, `curl`, and `tcpdump` provide practical visibility into different parts of Linux networking.
- DNS configuration can be affected by both global settings and per-connection NetworkManager settings.
- DNS-over-TLS encrypts DNS queries between the client resolver and the DNS provider.
- The current DNS tests showed **no evidence of an ISP DNS leak in the tested path**.
- The Quad9 protocol test returned `"dot"`, confirming that the tested query was handled through DNS-over-TLS.
- CSRF protection works at the HTTP request-validation layer and is separate from authentication and authorization.
- `{% csrf_token %}` produces a hidden form field, while Django's CSRF cookie stores the underlying secret used during validation.
- A CSRF token by itself is not proof of identity; CSRF protection specifically addresses forged state-changing browser requests.
- Inspecting HTTP responses directly made it possible to connect Django's template, cookie, middleware, and browser security behavior into one end-to-end flow.

### Progress / Updates

- Started the practical **Web & Networking Fundamentals** section of the training.
- Completed the first practical concepts covering the **client-server model, IP addresses, ports, TCP/UDP basics, ICMP, DNS, and HTTP request/response behavior**.
- Connected Linux networking concepts with the existing Django knowledge by inspecting real HTTP responses from the Django application.
- Completed a practical DNS investigation and verified the current Quad9 DoT path.
- Next topics planned: deeper TCP/UDP behavior, DNS resolution flow, HTTP methods and status codes, headers/cookies/bodies, HTTPS/TLS, browser DevTools Network tab, REST APIs, and WebSockets/SSE/polling.
